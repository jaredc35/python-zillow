...
class ValuationApi(object):
    ...
    def GetSearchResults(self, zws_id, address, citystatezip, rentzestimate=False):
        ...
        :param rentzestimat: Return Rent Zestimate information if available (boolean true/false, default: false)
        :return:
        """
    ...
    def GetUpdatedPropertyDetails(self, zws_id, zpid):
        ...
        xmltodict_data = xmltodict.parse(data)
        print (xmltodict_data.get('UpdatedPropertyDetails:updatedPropertyDetails',None)['response'])
        place = Place()
        # try:
        #     place.set_data(xmltodict_data.get('UpdatedPropertyDetails:updatedPropertyDetails',None)['response'])
        # except:
        #     raise ZillowError({'message': "Zillow did not return a valid response: %s" % data})
